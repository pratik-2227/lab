...
eqloc_initial = [10,5,20];   % assumed earthquake location
...
dgdt = 1;
dgdv = @(xr,yr,zr,xs,ys,zs,v) (1/(v.^2)).*(sqrt((xr-xs).^2 + (yr-ys).^2 + (zr-zs).^2 ));
...
% get eigenalues and eigenvectors of model covariance
% The eigen vectors will be the semi-major axis of the error ellipses
[u,lam] = eig(inv(C));
